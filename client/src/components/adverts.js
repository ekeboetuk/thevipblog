    ...
    return (
      <div className="pt-5 pt-md-0" style={{padding: '0 20px'}}>
        <div id="advert" ref={advertRef} className="container-md mt-md-5 position-relative" style={{width: '100%', height: '156px', opacity: '1', animation: 'reveal 2s linear', transition: 'visibility 2s ease-in-out, opacity 2s linear'}}>
          ...
          <i className="fa-solid fa-rectangle-xmark fa-lg position-absolute top-0 start-100 translate-middle text-brand" role="button" title="Click to hide banner" onClick={()=>{advertRef.current.classList.toggle('invisible'); advertRef.current.classList.toggle('opacity-0'); clearTimeout(timerRef.current)}}></i>
        </div>
      </div>
    )
}
