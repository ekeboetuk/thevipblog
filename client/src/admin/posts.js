...
function Posts() {
    const [posts, isError, isLoading, mutate] = usePosts('s/')
    ...
    if(isLoading) {
        ...
        if(isError.message.startsWith('timeout')) {
            return <h4>Network Timeout</h4>
        }else{
            return <Error />
        }
    } else {
        return (
            <div className="p-3 mb-0 mb-md-5 pe-md-0 " onClick={()=>setActionmenu({})}>
            <input type="text" id="search" className="bg-light w-100 mb-3 px-3 py-1" value={search} onChange={(e)=>handleSearch(e)} placeholder="Ajax Search" />
            <div className="d-flex flex-column flex-md-row justify-content-between mb-2">
                ...
                <div className="d-flex align-items-center mb-2">
                    ...
                    <div className="bg-light p-2 rounded-pill px-4">
                        <select id="postsort" className="border-0" name="postsort" value={sortby} onChange={(e)=>handlePostSort(e)}>
                            ...
                            <td className="border-0 p-3 bg-light">
                                <div className="post d-flex flex-column flex-md-row justify-content-between align-items-start position-relative">
                                    ...
                                    <div className="d-flex flex-column flex-grow-1 me-md-3">
                                        <Link to={`/post/${post.title}`} target="_blank"><h6 className="fw-bold text-black">{post.title} <i className="fa-solid fa-arrow-up-right-from-square fs-8"></i></h6></Link>
